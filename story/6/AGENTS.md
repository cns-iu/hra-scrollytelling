...
- Use `--story-viewport-height` for both a scene's outer ScrollTrigger-measured scroll-distance box (`.page-header-scene`, `.section2`, `.section3`, `section.transition`, `section.section5`) and its inner sticky stage that actually renders on screen (`.page-header` itself, `.fadeimage`, `.scene5-1`, `.transition__stage`, `.cde-tutorial-stage`). `layout.js` sets it to `window.innerHeight` plus a fixed safety margin on coarse pointers, and updates it only on load and once a genuine width-change resize (orientation) settles — never reactively on scroll, and never from a live `svh`/`lvh`/`dvh` CSS unit
    ...
    - The fixed safety margin is what actually solves the gap this value exists to prevent, without either failure mode: it makes the one-time-measured value generously larger than the true viewport even once the toolbar fully retracts, so it never needs to grow reactively. Overshooting only crops slightly off-screen on an `overflow: hidden` stage, which is invisible; only undershooting is visible, so err generous rather than precise
