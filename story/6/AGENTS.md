...
- Keep two separate viewport-height sources for two separate jobs; do not collapse them back into one, and do not derive either one from the other
    - `--story-viewport-height` sizes the **outer, ScrollTrigger-measured scroll-distance box** of every scene (`.page-header-scene`, `.section2`, `.section3`, `section.transition`, `section.section5`). `layout.js` updates it only on load and once a genuine width-change resize (orientation) settles — never reactively on scroll — so it stays a fixed number ScrollTrigger's cached start/end can rely on for the whole gesture. Updating it on every scroll settle previously left it stale for the entire duration of an active scroll (undersized relative to the real, currently-retracted toolbar), which both shrank each scene's effective scroll distance enough that a fast flick could blow through an entire reveal in too few frames to render, and reintroduced the geometry drift this value exists to prevent
    - The **inner sticky stage that actually renders on screen** (`.page-header` itself, `.fadeimage`, `.scene5-1`, `.transition__stage`, `.cde-tutorial-stage`) sizes its own `height` with a live `height: 100vh; height: 100dvh;` fallback pair instead, never `var(--story-viewport-height)`. GSAP never measures this inner element directly — only the outer box — so a live, continuously-repainted value is safe here and is in fact required: it is what keeps the visible stage filling the actual current viewport with no gap underneath it as the toolbar retracts mid-scroll, which the necessarily-stale outer value cannot do on its own
