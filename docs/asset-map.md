...
| `story/4/images/*.webp` | Corresponding `.png` in the same directory | Generated with `npm run webp:story4 -- --browser=/path/to/chromium`; the PNG is the retained master, since regenerating from a lossy file compounds the loss |
| `story/4/images/*.png` | Story 4 illustration rasters | Downscaled in place to roughly twice their rendered size; regenerate with `npm run images:story4`, which is idempotent and skips any file the resample would enlarge |
