...
| `prototypes/scrollytelling-effects/images/` | Scrollytelling Effects artwork and backgrounds | Organized; prototype only; the three filenames that contained spaces were renamed to hyphenated equivalents |
...
| `story/2/images/` | Story 2-owned narrative and quiz images | Organized; contains character scenes, ASCT+B Reporter artwork, quiz icons, and `squiggy-cilia-animation.gif`, extracted from the page where it had been inlined as base64 and mislabelled `data:image/png`; obsolete resource thumbnails were removed after the shared card migration |
...
| `story/3/images/` | Story 3-owned narrative images | Organized; contains character scenes, collision-state artwork, kidney variations, and the three RUI and tissue-block rasters extracted from the page's base64 data URIs; obsolete resource thumbnails were removed after the shared card migration |
...
| `story/3/images/rui-interface-registration.png`, `rui-interface-collision.png`, `tissue-block-detail.png` | Extracted from the base64 data URIs formerly inline in `story/3/index.html` | Two were downscaled to roughly twice their rendered size with `tools/png-codec.mjs`; the third was already below that target |
| `story/2/images/squiggy-cilia-animation.gif` | Extracted from the base64 data URI formerly inline in `story/2/index.html` | Animated GIF; the markup had declared it `data:image/png` |
| `story/4/images/*.webp` | Corresponding `.png` in the same directory | The 21 files the story actually serves; every `xlink:href` in `story/4/index.html` points here. Generated once with `npm run webp:story4 -- --browser=/path/to/chromium`, which also rewrote the markup from `.png` to `.webp`. That generator is now spent: it selects sources by their `.png` reference in the markup, so a second run finds none and exits with `No referenced PNG sources found` |
| `story/4/images/*.png` | Story 4 illustration rasters | **Unreferenced archival masters, not live assets.** 21 of the 22 are no longer named by any markup, stylesheet or script; only `scene8-pub-med.png` is still served, because its WebP came out larger. They are retained deliberately so a future re-encode need not start from a lossy source (4.3 MB against the 2.0 MB actually served). `npm run images:story4` is likewise spent: it derives its targets from `<image xlink:href="images/*.png">`, which `webp:story4` rewrote, so it now rewrites nothing |
| `story/6/images/*-{320,640,660,1280,1320}.png` | Corresponding full-resolution tissue, mouse, or tutorial PNG in the same directory | Generated responsive derivatives; regenerate with `npm run images:story6` |
...
| Story 1 | Organized | Independent of the former root `style.css`; uses runtime shared end matter, the complete shared Menu with appearance and High contrast controls, shared story navigation, and the shared footer; `story/1/` owns Story 1 presentation, motion behavior, image, videos, and end-matter content; animated media shared with a prototype is under `shared/assets/` |
| Story 2 | Presentation, runtime, and images organized | Uses the shared semantic narrative and flowing accessibility foundations, character-dialogue system, runtime end matter, two-link story navigation, complete shared Menu with appearance and High contrast controls, and the shared footer; `story/2/` owns its story-specific presentation, animation runtime, quiz tokens, end-matter content, and confirmed story-specific images; its embedded SVG labels specify the shared self-hosted Nunito Sans |
| Story 3 | Presentation, runtime, and images organized | Uses the shared semantic narrative and flowing accessibility foundations, character-dialogue system, runtime end matter, two-link story navigation, complete shared Menu with appearance and High contrast controls, and the shared footer; `story/3/` owns its split general and RUI animation modules, story-specific presentation, end-matter content, and confirmed images, while cross-story artwork is under `shared/assets/images/`; 18 pre-existing repeated ID values remain deferred with its embedded SVG data |
| Story 4 | Presentation and runtime organized | Independent of the former root `style.css`; uses a readable static default, a guarded enhanced layout, an ambient-animation control, runtime end matter, shared two-link story navigation, complete shared Menu with appearance and High contrast controls, and the shared footer; `story/4/` owns its presentation, particle scripts, focused animation modules, JSON configuration, and end-matter content; its embedded SVG identifiers are unique and its animation runtime is fetched only when the motion gate allows it |
| Story 5 | Presentation, runtime, and media organized | Uses the shared semantic narrative and flowing accessibility foundations, character-dialogue system, runtime end matter, two-link story navigation, complete shared Menu with appearance and High contrast controls, and the shared footer; reduced-motion and narrow layouts expose native video controls, while enhanced layouts provide visible play/pause controls; `story/5/` owns its story-specific animation and media-control modules, presentation, accessibility stylesheet, end-matter content, confirmed story-specific images, and videos |
