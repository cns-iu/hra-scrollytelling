...
| `stories/story3/styles.css` | Story 3 scene and embedded-artwork presentation | Organized; Story 3 still loads root `style.css` for the remaining legacy base, bottom-navigation, and resource-card rules |
...
| Story 3 | Presentation and images organized | Uses the shared narrative foundation and character-dialogue system, navigation-only shared Menu, and fixed-Dark shared footer; `stories/story3/` owns its story-specific presentation and confirmed images, while cross-story artwork is under `shared/assets/images/`; root `style.css` remains loaded only for the legacy base, bottom navigation, and resource cards; embedded data remains deferred |
